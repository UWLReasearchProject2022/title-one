...
def upload_developer():
    ...
    for name in DEV_NAMES:
        requests.post(BASE_URL + url, json={"name": name, "description": "A great company"})
...
GENRES = ["Action", "Adventure", "RPG", "Strategy", "Simulation", "Sports", "Racing", "Fighting", "MMO", "Shooter", "Horror",]
...
def upload_product():
    ...
    for name in PRODS:
        r = requests.post(BASE_URL + url, json={"name": name, "short_description": "A great product", "long_description": "A great product", "image_url": "asdf","developer_id": random.choice(developer_ids) })
        r.raise_for_status()
...
def upload_genre():
    url = "/genre/"
    for name in GENRES:
        r = requests.post(BASE_URL + url, json={"name": name, "description": "A great genre"})
        r.raise_for_status()
def check_db_empty():
    url = "/developer/"
...
def upload_product_platform():
    ...
    for prod in prod_ids:
        ...
        for i in range(random.randint(1,len(platform_ids))):
            response = requests.post(BASE_URL + url, json={"product_id": prod, "platform_id": platform_ids[i], "price": random.choice(PRICES)})
...
def upload_product_genre():
    ...
    for prod in PRODS:
        ...
        for i in range(random.randint(1,len(GENRES))):
            ...
            requests.post(BASE_URL + url, json={"product_id": prod, "genre_id": GENRES[i]})
...
def upload_stock():
    ...
    for plat in platform_ids:
        for _ in range(random.randint(1,10)):
            r = requests.post(BASE_URL + url, json={"product_platform_id": plat, })
            r.raise_for_status()
...
def upload_customer():
    ...
    for i in range(random.randint(1,10)):
        requests.post(BASE_URL + url, json={"surname": f"Customer {str(i)}", "email": "customer" + str(i) + "@gmail.com", "password": "password", "address": "address", "phone_number": "phone", "city": "LONDON", "other_names": "James"})
...
def upload_order():
    ...
    for i in range(random.randint(1,10)):
        requests.post(BASE_URL + url, json={"customer_id": random.choice(customer_ids), "date_ordered": "2020-01-01","date_shipped": "2020-01-01" , "date_delivered": "2020-01-01", })
